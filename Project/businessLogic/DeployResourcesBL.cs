    ...
    public class DeployResourcesBL
    {
        ...
        public static void Bind(Repeater rpt)
        {
            try
            {
                ...
                string SqlString = " SELECT AllocationID,[dbo].[ReportingManager](CPT_ResourceDemand.ResourceRequestBy) as RequesterEmail,CPT_AllocateResource.RequestId,CPT_ResourceMaster.EmployeeMasterID, " +
                                   "[dbo].[Owner](CPT_ResourceDemand.ResourceRequestBy) as Requested_By, CPT_ResourceMaster.EmployeetName, ISNULL(CAST(CPT_AllocateResource.StartDate AS VARCHAR(12)), '-') AS StartDate, " +
